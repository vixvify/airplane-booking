...
แต่ละ client รับคำสั่งจาก stdin และรอ response ใน System V queue เดียวกันกับ request โดย request ใช้ message type ค่าสูงสุดของ `long` ส่วน response ใช้ `responseType` ที่สร้างเฉพาะสำหรับคำสั่งนั้น จึงไม่รับคำตอบสลับกันแม้ client ID ซ้ำ
...
Server ใช้ System V queue เดียวที่สร้างจาก `ftok("/ipc", 'A')`: request ใช้ message type ค่าสูงสุดของ `long` ส่วน response ใช้ `responseType` เฉพาะของคำสั่งใน queue เดียวกัน Server จะล็อก `/ipc/server.lock` ก่อนจัดการ queue: เปิดซ้ำจะถูกปฏิเสธโดยไม่กระทบตัวเดิม แต่หลัง crash ยังลบ stale queue และเริ่มใหม่ได้
...
- หาก client timeout หลังส่ง request ผลของ operation ยังไม่แน่นอน ให้ตรวจ `STATUS` ก่อน retry; request กำหนด deadline ให้ response และ worker จะทิ้ง response ที่หมดอายุ เพื่อไม่ให้ค้างเต็ม shared queue
